...
def generate_recommendation(sql_cursor,sql_db,user, activity_category, n):
    '''
    generate an activity recommendation 
    '''

    #Extract the user's latest fitness level & activity preferences
    user_prefs,user_fitness=get_user_metadata(sql_cursor, user)
    ...
    vals=(activity_category,user_fitness, user_fitness)
    sql_cursor.execute(query,vals)
    ...
    for index,row in user_prefs.iterrows():
        ...
        hits=matchers[cur_feature](cur_feature_value, cur_feature, activity_cur_feature_vals, sql_cursor, user)
    ...
    selected_hashes=rank_options(options,n)
...
def make_output(activity_hashes, activity_names, activity_category, outf=None):
    '''
    return activity selection string 
    '''
    if outf is None:
        print('specified category:'+ activity_category)
        for i in range(len(activity_hashes)): 
            print(activity_hashes[i]+'\t'+activity_names[i])
    else:
        outf=open(outf,'w')
        outf.write('specified category:'+activity_category+'\n')
        for i in range(len(activity_hashes)):
            outf.write(activity_hashes[i]+'\t'+activity_names[i]+'\n')
        outf.close()


def recomend_main_helper(user, activity_category, n, outf=None):
    #create mysql connection
    sql_db,sql_cursor =open_mysql_connection()

    if activity_category=="skip":
        #raise(Exception, "No longer supporting skip")
        #user selected to skip exercise
        make_output(['skip']*n,['skip']*n, activity_category, outf)
        recommended_activity_hashes="skip"
        recommended_activity_names = "skip"
    else:
        #recommend an exercise
        recommended_activity_hashes, recommended_activity_names=generate_recommendation(sql_cursor,sql_db,user, activity_category, n)
        make_output(recommended_activity_hashes, recommended_activity_names,activity_category, outf)
...
def main():
    ...
    args=parse_args()
    recomend_main_helper(args.user, args.activity_category, args.n, args.outf)
